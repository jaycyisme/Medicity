<div class="table-responsive">
    ...
    <table class="table table-bordered">
        ...
        <tbody>
            ...
            <tr>
                <td class="align-middle"><img class="rounded-circle img-fluid wid-60 img-thumbnail" src="@if(isset($product->thumbnail)){{ asset('storage/Product/'.$product->thumbnail) }}@else{{ asset('assets/images/user/avatar-1.jpg') }}@endif" alt="{{ $product->name }}"></td>
                <td class="align-middle">{{ $product->name }}</td>
                <td class="align-middle">{{ $product->category ? $product->category->name : 'Trống' }}</td>
                ...
                <td class="align-middle">
                    <div class="d-flex align-items-center">
                        ...
                        <div class="p-1 text-center">
                            <a class="btn btn-sm btn-warning wid-50" href="{{ route('product.show', $product->id) }}" data-bs-toggle="tooltip" data-bs-placement="top" title="Xem">
                        ...
                        {{-- @can('product-edit') --}}
                        <div class="p-1 text-center">
                            <a class="btn btn-sm btn-info wid-50" href="{{ route('product.edit', $product->id) }}">
                                ...
                                <div class="address-check border rounded p-3 d-flex justify-content-between">
                                    <div class="form-check">
                                        <label class="form-check-label d-block" for="address-check-1">
                                            ...
                                            <span class="h6 mb-0 d-block">{{ $product->name }}</span>
                                        </label>
                                    </div>

                                    <div class="action">
                                        <button wire:click.prevent='restore({{ $product->id }})' class="btn btn-sm btn-info">
                                            <i class="ph-duotone ph-rewind"></i>
                                        </button>
                                        <button wire:click.prevent='forceDelete({{ $product->id }})' class="btn btn-sm btn-danger">
