<div class="table-responsive">
    ...
    <table class="table table-bordered">
        ...
        <tbody>
            ...
            <tr>
                <td class="align-middle"><img class="rounded-circle img-fluid wid-60 img-thumbnail" src="@if(isset($service->thumbnail)){{ asset('storage/Clinic/'.$service->thumbnail) }}@else{{ asset('assets/images/user/avatar-1.jpg') }}@endif" alt="medicity"></td>
                <td class="align-middle">{{ $service->name }}</td>
                <td class="align-middle">
                    <div class="d-flex align-items-center">
                        {{-- @can('service-list') --}}
                        {{-- <div class="p-1 text-center">
                            <a class="btn btn-sm btn-warning wid-50" href="{{ route('service.show', $service->id) }}" data-bs-toggle="tooltip" data-bs-placement="top" title="Xem">
                        ...
                        {{-- @can('service-edit') --}}
                        <div class="p-1 text-center">
                            <a class="btn btn-sm btn-info wid-50" href="{{ route('service.edit', $service->id) }}">
                ...
                <div class="modal-body">
                    <div class="collapse multi-collapse show">
                        <div class="address-check-block">
                            ...
                            {{-- @foreach ($clinics_deleted as $clinic)
                                        ...
                                        <label class="form-check-label d-block" for="address-check-1">
                                            <div class="chat-avtar d-inline-flex mx-auto">
                                                <img class="rounded-circle img-fluid wid-90 img-thumbnail"
                                                    src="@if (isset($clinic->image_url)) {{ asset('storage/Clinic/' . $clinic->image_url) }}@else{{ asset('assets/images/user/avatar-1.jpg') }} @endif"
                                                    alt="chillnfree">
                                            </div>
                                            <span class="h6 mb-0 d-block">{{ $clinic->name }}</span>
                                        </label>
                                    </div>

                                    <div class="action">
                                        <button wire:click.prevent='restore({{ $clinic->id }})' class="btn btn-sm btn-info">
                                            <i class="ph-duotone ph-rewind"></i>
                                        </button>
                                        <button wire:click.prevent='forceDelete({{ $clinic->id }})' class="btn btn-sm btn-danger">
