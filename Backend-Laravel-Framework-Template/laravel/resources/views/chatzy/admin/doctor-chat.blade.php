    ...
    <div class="">
                                                                    ...
                                                                    <div class="card shadow-sm">
                                                                        <div class="card-header bg-primary text-white">
                                                                            <div class="d-flex align-items-center">
                                                                                @if ($selectedUser)
                                                                                    <img id="chat_img" src="@if(isset($appointment->doctor->profile_photo_path)){{ asset('storage/profile-photos/'.$appointment->doctor->profile_photo_path) }}@else{{ asset('assets/images/user/avatar-1.jpg') }}@endif" class="rounded-circle mr-3" alt="Profile Picture" style="width: 40px; height: 40px;">
                                                                                @endif
                                                                                <h4 class="mb-0" id="chat_name">{{ isset($selectedUser) ? 'Chatting with ' . $selectedUser->name : 'Please select a user to chat' }}</h4>
                                                                            </div>
                                                                        </div>
        ...
        <script>
            $(document).ready(function () {
                let selectedUserId = "{{ isset($selectedUser) ? $selectedUser->id : '' }}";
                let selectedUserName = "{{ isset($selectedUser) ? $selectedUser->name : '' }}";

                function loadMessages(userId, userName) {
                    $('#receiver_id').val(userId);
                    $('#chat_name').text('Chatting with ' + userName);
                            ...
                            response.messages.forEach(function(message) {
                                ...
                                let messageHtml = `
                                    <div class="chat-message ${isSender ? 'sender' : 'receiver'}">
                                        <div class="message-content">
                                            <p><strong>${isSender ? 'You' : userName}:</strong> ${message.message}</p>
                                            <div class="timestamp">${messageTime}</div>
                                        </div>
                                    </div>`;
                ...
                // Nếu đã có User được chọn khi vào trang, tự động hiển thị tin nhắn
                if (selectedUserId) {
                    loadMessages(selectedUserId, selectedUserName);
                }

                // Xử lý sự kiện khi chọn User từ danh sách
                $(document).on('click', '.chat-item', function () {
                    let userId = $(this).find('.id').text();
                    let userName = $(this).find('.profile_name').text();

                    $('.chat-item').removeClass('active'); // Xóa class active khỏi các User khác
                    $(this).addClass('active'); // Thêm class active vào User đang chọn

                    loadMessages(userId, userName); // Tải tin nhắn của User được chọn
                });
            });
        </script>
        <script>
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            response.messages.forEach(function(message) {
                                ...
                                let userAvatar = isSender ? "{{ Storage::url($LoggedDoctorInfo->picture) }}" : profileImage;
                                let userName = isSender ? '{{ $LoggedDoctorInfo->name }}' : profileName;
                                ...
                                let messageHtml = `
                                    <div class="chat-message ${isSender ? 'sender' : 'receiver'}">
                                        <div class="message-content">
                                            <p ${isSender ? 'style = color: white;' : ''}><strong>${userName}:</strong> ${message.message}</p>
                                            <div class="timestamp">${messageTime}</div>
                                        </div>
                                    </div>`;
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response.success) {
                                ...
                                let userAvatar =
                                    '{{ asset('storage/' . $LoggedDoctorInfo->picture) }}';
                                let userName = '{{ $LoggedDoctorInfo->name }}';
                        ...
                        <div class="chat-message sender">
                            <div class="message-content">
                                <p style = color: white;><strong>${userName}:</strong> ${message}</p>
                                <div class="timestamp">${messageTime}</div>
                            </div>
                        </div>`;
